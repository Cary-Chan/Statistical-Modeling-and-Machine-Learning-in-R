...
#input the data
data<-read.table("/Users/chenchuandong/Desktop/german.csv",header = TRUE, sep = ",", dec=".")
summary(data)
colnames(data)
nrow(data)
head(data)
...
DecisionTree<-rpart(training$target~.,training)
summary(DecisionTree)

#plot the tree
rpart.plot(DecisionTree,type=3,extra="auto",digits=4,Margin=0)

#prediction
pr<-predict(DecisionTree,testing)
...
ppe(testing$target, predType)

#plot roc
library(ROCR)
pred<-prediction(testing$target, predType)
